...
//
// T19.3: enqueue/generate carry BOTH the Uzbek word (cache key + Anki first
// field) and the English gloss (image prompt). The card stays IMAGE:UZBEK; the
// prompt runs on English for concept fidelity.

/// The offline queue `type` for AI image generation. The QueueWorker handler
/// registered under this key decodes a JSON `{"uzbek","english"}` payload and
/// calls [AiImageService.generateNow].
...
abstract class AiImageService {
  ...
  /// Ensure an image will be generated for the pair. Idempotent: a no-op
  ...
  /// call offline — enqueue never blocks on the network.
  ...
  Future<void> generateNow({required String uzbek, required String english});
}
