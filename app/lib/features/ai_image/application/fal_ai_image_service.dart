...
// rendering is a non-issue). The API key is supplied at runtime via
// --dart-define (RIVENDELL_FAL_KEY); it never lives in the repo (NFR: no
...
class FalAiImageService implements AiImageService {
  ...
  Future<void> generateNow(String uzbekWord) async {
    ...
    if (apiKey.isEmpty) {
      ...
      // next reconnect, which won't help until a key is supplied. That retry
      // storm is bounded by queue-hardening (dead-letter after N attempts,
      // follow-up #16); for now the message names the missing define.
      throw StateError(
        'RIVENDELL_FAL_KEY not configured; cannot generate image for "$word"',
      );
    }

    final imageUrl = await _requestImageUrl(word);
  ...
  Future<String> _requestImageUrl(String word) async {
